...
// Known formats with high confidence are classified as STABLE.
// Unrecognized formats are classified as UNKNOWN (ready for future Python AI escalation).
func (d *DefaultDriftDetector) Analyze(ctx context.Context, event models.RawEvent, detection models.DetectionResult) (models.DriftResult, error) {
	if detection.Format == FormatUnknown || detection.Confidence < 0.5 {
	...
	return models.DriftResult{
		...
		Confidence:      detection.Confidence,
		Message:         "Payload matches established " + detection.Format + " parser contract",
